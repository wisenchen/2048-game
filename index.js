...
var lock = true; // 在一次操作未完成的时候需要锁住，避免连续操作
var isChange = false; // 是否产生变化
var gameStatus = 0; // 游戏状态 0: 未开始 1: 进行中 2: 结束
var maxScore = localStorage.getItem("2048_maxScore") || 0; // 最高分
  ...
  for (var i = 0; i < newSquareSet.length; i++) {
    for (var j = 0; j < newSquareSet[i].length; j++) {
      ...
      newSquareSet[i][j].style.left =
        (j + 1) * spacing + j * squareWidth + "px"; //j+1个间隙加上j个方块宽度
...
function updateScore() {
  ...
  for (var i = 0; i < squareSet.length; i++) {
    for (var j = 0; j < squareSet[i].length; j++) {
      var item = squareSet[i][j];
      score += (item && item.num) || 0;
    }
  }
  if (score >= maxScore) {
...
function updateMaxScore(score) {
  if (score) {
    localStorage.setItem("2048_maxScore", score);
  }
  var maxScore = localStorage.getItem("2048_maxScore") || 0;
